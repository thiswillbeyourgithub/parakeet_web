...
| 📦 **SmoothQuant int8 Encoder** | The encoder runs as a SmoothQuant int8 model by default, recalibrated so its accuracy tracks fp32 even on long audio (unlike a stock int8 cast that degrades badly past ~30 s). From the encoder-precision picker you can opt into the full **sharded fp32** encoder (~2.4 GB, about 35 % slower on the CPU path as measured on the reference machine, split into <2 GB pieces by `scripts/shard-fp32.py` so it fits the browser) for maximum quality, or the **int8 lite** build in the other direction (same calibration, 11 matrix multiplications kept in fp32 instead of 18: about 88 MB less to download and about 164 MiB less peak memory, for slightly more error). A third option, **w4a8**, stores the weights on 4 bits: about 610 MB instead of 2,436 MB and the fastest of the four to load, at the same accuracy as int8, but it transcribes slower (about 2.6x real time on the CPU path against the same model's fp32 at 3.6x, and int8 is faster than either) because the encoder is limited by arithmetic rather than by memory traffic. Any of them stops with a clear message rather than silently downgrading when the model repo doesn't ship the matching files. The decoder always runs int8 (on this model the int8 joiner is as accurate as fp32, while being smaller and faster). On the WebGPU backend the encoder is fp32 or w4a8: the GPU execution provider has no int8 encoder kernel, but it does have one for the 4-bit weights |
