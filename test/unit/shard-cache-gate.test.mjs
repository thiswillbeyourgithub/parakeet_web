...
// ---------------------------------------------------------------------------
// Minimal fake IndexedDB (see hub-cache-readback.test.mjs for the rationale)
// ---------------------------------------------------------------------------
...
function makeStoreApi() {
  return {
    get(key) {
      ...
      later(() => { request.result = fakeIdb.data.get(key); if (request.onsuccess) request.onsuccess(); });
      return request;
    },
    put(value, key) {
      const request = makeRequest();
    ...
    delete(key) {
      const request = makeRequest();
      later(() => { fakeIdb.data.delete(key); if (request.onsuccess) request.onsuccess(); });
      return request;
    },
    getAllKeys() {
      const request = makeRequest();
      later(() => { request.result = [...fakeIdb.data.keys()]; if (request.onsuccess) request.onsuccess(); });
      return request;
    },
    clear() {
      const request = makeRequest();
      later(() => { fakeIdb.data.clear(); if (request.onsuccess) request.onsuccess(); });
      return request;
    },
  };
}
...
function makeFakeDb(version) {
  ...
  return {
    ...
    close() {},
    transaction() { return { objectStore: () => storeApi }; },
  };
}
