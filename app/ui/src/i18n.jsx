...
const translations = {
  en: {
    ...
    tooltipParallelEncode: 'Encode several audio chunks at once in background workers while the main thread decodes, and move decoding into its own worker too when possible, roughly 30-45% faster on long audio. Costs extra RAM (each worker keeps its own ~850 MB copy of the model), so it is skipped automatically on machines with under 4 cores or under 8 GB of memory, and when a chunk-parallel run fails it falls back to the normal path. Default: on.',
    relaxedSimd: 'Relaxed-SIMD engine',
    tooltipRelaxedSimd: 'Runs the speech engine on a Relaxed-SIMD build of ONNX Runtime, which speeds up the int8 matrix math on browsers whose engine profits from it. Only shown when this deployment ships the alternate build and your browser supports it. Auto (the default) runs a quick benchmark at the first model load and picks the faster engine for your browser; On and Off force the choice. Applied at the first model load of the page: after changing it, reload the page.',
    relaxedSimdAuto: 'Auto (benchmark)',
    relaxedSimdOn: 'On',
    relaxedSimdOff: 'Off',
  ...
  fr: {
    ...
    tooltipParallelEncode: "Encode plusieurs segments audio à la fois dans des workers en arrière-plan pendant que le fil principal décode, et déplace aussi le décodage dans son propre worker quand c'est possible : environ 30 à 45 % plus rapide sur les audios longs. Consomme plus de RAM (chaque worker garde sa propre copie du modèle, ~850 Mo), donc désactivé automatiquement sous 4 cœurs ou sous 8 Go de mémoire ; en cas d'échec, le traitement retombe sur le chemin normal. Défaut : activé.",
    relaxedSimd: 'Moteur Relaxed-SIMD',
    tooltipRelaxedSimd: 'Exécute le moteur vocal sur une version Relaxed-SIMD de ONNX Runtime, qui accélère le calcul matriciel int8 sur les navigateurs dont le moteur en tire profit. Visible uniquement quand ce déploiement fournit la version alternative et que votre navigateur la prend en charge. Auto (le défaut) lance un bref benchmark au premier chargement du modèle et choisit le moteur le plus rapide pour votre navigateur ; Activé et Désactivé forcent le choix. Appliqué au premier chargement du modèle de la page : après un changement, rechargez la page.',
    relaxedSimdAuto: 'Auto (benchmark)',
    relaxedSimdOn: 'Activé',
    relaxedSimdOff: 'Désactivé',
