...
const translations = {
  en: {
    ...
    tooltipEncoderPrecision: 'This sets the ENCODER precision only, not the whole model: the fused decoder/joiner always runs int8 (as accurate as fp32 here, but smaller and faster). int8 (WASM only): small, fast, and (with the SmoothQuant encoder) good quality on long audio. fp32: maximum quality, but ~2.4 GB (shipped as <2GB shards) and about 35 % slower on the CPU path. fp32 is also the only precision WebGPU has, since the GPU has no int8 encoder kernel. It applies only if the model repo provides the shards; otherwise the load stops with a clear message rather than silently downgrading.',
  ...
  fr: {
    ...
    tooltipEncoderPrecision: "Ceci d\u00e9finit uniquement la pr\u00e9cision de l'ENCODEUR, pas le mod\u00e8le entier\u00a0: le d\u00e9codeur/joiner fusionn\u00e9 tourne toujours en int8 (aussi pr\u00e9cis que fp32 ici, mais plus l\u00e9ger et plus rapide). int8 (WASM uniquement)\u00a0: petit, rapide, et (avec l'encodeur SmoothQuant) de bonne qualit\u00e9 sur les longs audios. fp32\u00a0: qualit\u00e9 maximale, mais ~2,4\u00a0Go (livr\u00e9 en fragments de moins de 2\u00a0Go) et environ 35\u00a0% plus lent sur la voie CPU. fp32 est aussi la seule pr\u00e9cision disponible sur WebGPU, le GPU n'ayant pas de noyau d'encodeur int8. Elle ne s'applique que si le d\u00e9p\u00f4t du mod\u00e8le fournit les fragments\u00a0; sinon le chargement s'arr\u00eate avec un message clair plut\u00f4t qu'une r\u00e9trogradation silencieuse.",
