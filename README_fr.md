...
| 🧵 **Encodage parallèle** | Sur les enregistrements longs, les segments audio sont encodés en parallèle dans des workers en arrière-plan pendant que le fil principal décode, environ 30 à 45 % plus rapide au total. Activé par défaut sur les machines capables (4 cœurs et plus, 8 Go de RAM et plus ; cela consomme plus de mémoire car chaque worker garde sa propre copie de l'encodeur) et désactivable dans les réglages |
