...
- [Performances et matériel](#performances-et-matériel)
...
| 📦 **Encodeur int8 SmoothQuant** | L'encodeur tourne en int8 SmoothQuant par défaut, recalibré pour que sa précision suive celle de fp32 même sur de l'audio long (contrairement à une conversion int8 standard qui se dégrade fortement au-delà d'environ 30 s). Depuis le sélecteur de précision de l'encodeur, vous pouvez opter pour l'encodeur **fp32 fragmenté** complet (~2,4 Go, environ 35 % plus lent sur la voie CPU d'après les mesures sur la machine de référence, découpé en morceaux de moins de 2 Go par le `scripts/shard-fp32.py` du dépôt du modèle pour qu'il tienne dans le navigateur, et partiellement conservé entre deux chargements : un navigateur refuse de relire plus d'environ 2,1 Go de poids stockés, donc 1,5 Go de l'ensemble est mis en cache et le reste est retéléchargé, ce qui ramène un chargement suivant à environ 0,95 Go transférés au lieu de 2,4 Go) pour une qualité maximale, ou pour la version **int8 lite** dans l'autre sens (même calibration, 11 multiplications matricielles laissées en fp32 au lieu de 18 : environ 88 Mo de téléchargement en moins et environ 164 Mio de mémoire de pointe en moins, au prix d'un peu plus d'erreur). Une troisième option, **w4a8**, stocke les poids sur 4 bits : environ 610 Mo au lieu de 2 436 Mo et le plus rapide des quatre à charger, avec la même précision qu'int8, mais il transcrit plus lentement (environ 2,6x le temps réel sur la voie CPU contre 3,6x pour le fp32 du même modèle, et int8 est plus rapide que les deux) car l'encodeur est limité par le calcul et non par les accès mémoire. Chacune de ces options s'arrête avec un message clair plutôt qu'une rétrogradation silencieuse lorsque le dépôt du modèle ne fournit pas les fichiers correspondants. Le décodeur tourne toujours en int8 (sur ce modèle, le joiner int8 est aussi précis que fp32, tout en étant plus léger et plus rapide). Sur le backend WebGPU, l'encodeur est en fp16 par défaut, sinon en fp32 ou en w4a8 : le fournisseur d'exécution GPU n'a pas de noyau d'encodeur int8, mais il en a un pour les poids sur 4 bits, et un pour fp16 (sans perte, ~1,2 Go, moitié moins que fp32), ce qui en fait ce qu'un visiteur obtient avant d'avoir choisi quoi que ce soit. Il n'est utilisé que sur un GPU qui annonce la fonctionnalité `shader-f16`, car sans elle le modèle se construit puis renvoie une transcription vide, et uniquement quand la source de modèles héberge le fichier ; chacun de ces deux refus bascule le chargement sur le CPU en int8 avant le moindre octet téléchargé, jamais sur fp32 ni sur w4a8, qui ne s'obtiennent qu'à la main (l'un triple presque un téléchargement non demandé, l'autre est le plus faible des encodeurs sur les longs enregistrements). Une précision que ce déploiement ne peut pas fournir n'est pas proposée du tout plutôt que découverte à la fin d'un chargement, et quand int8 lui-même ne se charge depuis aucune source, l'application s'arrête sur un message bloquant qui nomme ce qui manque plutôt que sur un simple « Échec ») |
...
**Pendant l'exécution.** Le banc d'essai emprunte les mêmes chemins de chargement et de transcription que vous : les commandes d'enregistrement et d'envoi de fichier disparaissent donc le temps de l'exécution, car une capture ne ferait que disputer au banc d'essai le modèle qu'il est en train de mesurer. À la fin, la barre latérale se rouvre sur les résultats et défile jusqu'au rapport. Si aucun modèle n'était chargé au départ, aucun ne reste chargé à l'arrivée (les poids en mémoire seraient ceux de la dernière combinaison mesurée, pas ceux que vos réglages affichent) : le bouton de chargement du modèle réapparaît, et le prochain chargement sera celui que vous avez demandé.

**Le tableau.** Le tableau complet s'affiche dès que vous lancez le test : toutes les lignes prévues sont là dès le départ, marquées en attente, puis indiquent ce qu'elles font (chargement, transcription) pendant qu'elles le font, puis se figent sur leur résultat. Une ligne déjà mesurée conserve son résultat quoi qu'il advienne du reste du test. La colonne de vitesse indique de l'audio par seconde de calcul : 5x signifie cinq minutes d'enregistrement traitées en une minute, et plus grand vaut mieux. Les combinaisons que cette instance ne peut pas fournir sont retirées du plan plutôt que mesurées jusqu'à l'échec, et un chargement de banc d'essai n'accepte jamais de précision de substitution : une ligne doit être la précision qu'elle annonce, ou dire qu'elle n'a pas pu s'exécuter.
...
**Ce que contient le rapport :** les mesures de temps (chargement du modèle, temps total, temps par seconde d'audio, répartition encodage/décodage), ce que votre processeur et votre GPU prennent en charge (nombre de cœurs, classe de mémoire, architecture et plateforme du processeur, SIMD et threads WASM, tous les adaptateurs WebGPU que votre navigateur veut bien exposer avec leurs fonctionnalités et leurs limites, les noms de GPU rapportés par votre navigateur via WebGL, la configuration ONNX Runtime en vigueur), les réglages de l'application qui changent la vitesse (threads, largeur du faisceau, fenêtre de découpage, renforcement de phrases actif ou non), et un score de similarité de chaque transcription avec la phrase connue de l'extrait, ce qui permet de détecter un moteur qui renvoie silencieusement du vide. Sur un portable équipé de deux GPU, le GPU intégré et le GPU dédié sont tous les deux listés : une mesure que personne ne peut rattacher à une puce ne répond à rien. Aucun navigateur n'expose le modèle du processeur, à qui que ce soit : c'est la seule chose qu'un rapport ne peut pas contenir.
...
Les rapports arrivent sur `POST /api/signal/benchmark-report` du service de signalisation : limité à 3 par minute et par IP et à 30 par minute pour toute l'instance (`BENCHMARK_REPORTS_MAX_PER_MINUTE`, pour qu'une inondation depuis des adresses tournantes ne puisse pas remplir le dossier en quelques minutes), plafonné à 32 Ko chacun et à 20000 fichiers au total, avec vérification de la forme (champs connus seulement, aucune clé polluant le prototype, profondeur et longueur des chaînes bornées, aucun caractère de contrôle, de sorte que le fichier stocké peut être fusionné ou affiché sans surprise), et stocké à raison d'un rapport par fichier sous un nom choisi par le serveur lui-même (rien venant de la requête n'atteint un chemin). Rien concernant l'expéditeur n'est écrit à côté du contenu, car un rapport anonyme dans le navigateur ne doit pas cesser de l'être à l'arrivée.
...
La même règle vaut dans l'autre sens, et il faut donc le dire clairement. En comparant la version 9.9.0 telle que publiée à la version 10.0.0 telle que publiée, sur le même enregistrement de 6,5 minutes, en alternant l'ordre des deux versions à chaque répétition, la voie GPU est devenue **50x plus rapide** (de 1004 s à 20 s) tandis que la voie CPU est devenue environ **7 % plus lente** (de 104 s à 111 s, 20 exécutions par version). Ce coût CPU n'est pas la somme de plusieurs changements qui coûteraient chacun un peu : tout ce que la 10.0.0 a changé au travail CPU en dehors de la fenêtre de découpage mesure +0,5 %, c'est-à-dire rien. La totalité vient du passage de la fenêtre de 20 s à 60 s, et c'était un choix délibéré, car une fenêtre plus longue signifie moins de coutures et environ un demi-point de WER de mieux. La fenêtre reste réglable dans les Paramètres si vous préférez récupérer ces 7 %.
...
La vraie cause était la page elle-même. Le moteur rend la main à la boucle d'événements environ 2000 fois par exécution de l'encodeur, et Chromium ne délivre ces rappels qu'au rythme où la page produit des images de composition, et ce pour tout le processus. Le simple indicateur d'attente animé taxait donc chacune de ces 2000 interruptions. Mettre en pause les animations de la page pendant toute la durée d'une exécution GPU a supprimé la totalité de cette taxe : le même extrait de 3 minutes est passé de 12 min 39 s à 8,5 s. Sur la machine de référence (une RTX 3090 Ti), WebGPU traite désormais un enregistrement de 6,5 minutes en environ 20 s contre environ 111 s pour WASM int8, soit environ 5x plus rapide au lieu de 15x plus lent.

WebGPU est donc de nouveau disponible. Il n'est pour autant pas sélectionné pour tout le monde, car « le GPU est-il plus rapide ici » est une question sur votre machine, pas sur ce modèle, et se tromper coûte cher dans un sens : la voie GPU télécharge 1,2 à 2,4 Go de poids au lieu d'environ 600 Mo. Plutôt que d'extrapoler d'un seul GPU mesuré à tous les visiteurs, l'application mesure la machine sur laquelle elle tourne et ne bascule vers le GPU qu'en cas de gain net (voir [Configuration automatique](#configuration-automatique--mesurer-plutôt-que-supposer) ci-dessous). Une machine sans GPU, ou dont le GPU ne peut pas exécuter le modèle, reste sur la voie CPU exactement comme avant. Et si la source du modèle ne fournit elle-même aucune version GPU de l'encodeur, ce à quoi ressemble une installation autohébergée pointée vers un dépôt de modèles CPU uniquement, le chargement bascule sur la version CPU et vous le signale, au lieu d'échouer.

Les boutons de précision décrivent eux aussi CETTE instance, et non les précisions que l'application connaît dans l'absolu. Pendant le chargement de la page, l'application demande à la source de modèles qu'un chargement utiliserait réellement ce qu'elle héberge, et grise les précisions que cette source ne peut pas fournir, chacune avec sa raison : le moteur ne sait pas l'exécuter, ce GPU n'annonce pas `shader-f16`, ou la source n'héberge pas le fichier. Ces raisons désignent trois personnes différentes (la version de l'application, votre matériel, l'administrateur de votre instance), elles ne sont donc jamais fondues en un seul message. Si la liste des fichiers ne peut pas être lue, toutes les précisions restent proposées exactement comme avant : une vérification incertaine ne doit pas retirer un choix qui aurait fonctionné.

Si la précision utilisée s'avère finalement absente de la source (un réglage enregistré devenu obsolète, un miroir qui a changé entre-temps), l'application cherche une autre précision que le GPU sait exécuter et que cette source héberge, en commençant par le plus petit téléchargement, et charge celle-là : vous conservez le moteur que vous avez choisi, et le message indique ce qui a été demandé, ce qui a été chargé et la taille approximative. Ce n'est que si aucune précision GPU ne peut être fournie que le repli vers le processeur a lieu.

Lorsqu'un repli se produit, les réglages suivent : le moteur bascule (il le doit, puisque la nouvelle tentative le relit) et la précision d'encodeur suit, de sorte que vos réglages enregistrés ne contredisent jamais discrètement ce qu'affiche le panneau. Le sélecteur de dépôt est délibérément laissé tel quel, pour qu'un changement de modèle que vous venez de demander ne soit pas annulé à votre insu.

Comme l'application a le droit de résoudre votre sélection autrement dans tous ces cas, le panneau de réglages affiche une ligne **Actuellement chargé** au-dessus des réglages de modèle dès qu'un modèle est en place : le moteur, la précision d'encodeur réellement montée, et si les poids viennent de HuggingFace ou de ce serveur. Les réglages en dessous décrivent ce que le PROCHAIN chargement demandera ; cette ligne décrit ce qui répond en ce moment, et le signale explicitement quand les deux divergent.
...
Savoir si un GPU bat le CPU pour ce travail dépend entièrement de la machine : l'application le mesure donc sur place au lieu de le supposer. Deux petits graphes ONNX (environ 5 Mo au total, livrés avec l'application) sont téléchargés discrètement en arrière-plan d'un chargement de page normal, et au premier chargement du modèle ils sont chronométrés sur les deux voies : celui en int8 via WASM, celui en fp32 via WebGPU. Le côté processeur correspond à ce que ce backend exécute vraiment ; le côté graphique est la plus lourde des deux précisions qu'il pourrait exécuter, si bien que la mesure sous-estime le GPU au lieu de le flatter, seule direction dans laquelle une erreur reste supportable ici. Le plus rapide gagne et est sélectionné avant le début du téléchargement, si bien que le verdict décide des poids que vous récupérez. La mesure est entièrement escamotée quand la carte graphique n'a rien que l'application y mettrait sans qu'on le lui demande (pas de `shader-f16`, ou une source de modèles sans fichier fp16) : inutile de dépenser deux exécutions chronométrées à prouver qu'une carte est plus rapide pour charger malgré tout sur le processeur.
...
```bash
# 4. Facultatif mais recommandé : précompressez les poids une fois pour toutes.
